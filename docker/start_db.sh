...
docker cp "$LOCATION"/jwtSecret configs:/cfg/jwtSecret
docker cp "$LOCATION"/server.pem configs:/cfg/server.pem

docker run --rm --volumes-from configs "$STARTER_DOCKER_IMAGE" auth header --auth.jwt-secret /cfg/jwtSecret > "$LOCATION"/jwtHeader
