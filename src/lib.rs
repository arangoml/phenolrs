...
#[pyfunction]
#[cfg(not(test))]
fn graph_to_json_format(py: Python, request: DataLoadRequest) -> PyResult<(&PyDict, &PyDict, &PyArray1<usize>, &PyArray1<usize>, &PyDict)> {
    ...
    let res = (node_dict, adj_dict, src_indices, dst_indices, vertex_id_to_index);
