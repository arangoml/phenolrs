...
pub struct NetworkXGraph {
    ...
    pub edge_values: Vec<HashMap<String, Vec<u64>>>, // {'weight': [4, 5, 1], ...)}
    ...
    insert_coo_fn: fn(&mut NetworkXGraph, String, String, HashMap<String, Vec<u64>>),
...
impl NetworkXGraph {
    ...
    fn insert_coo_graph(
        ...
        from_id_str: String,
        to_id_str: String,
        properties: HashMap<String, Vec<u64>>,
    ) {
        ...
        self.edge_values.push(properties);
    }
...
impl Graph for NetworkXGraph {
    ...
    fn insert_edge(
        ...
        let mut properties: HashMap<String, Vec<u64>> = HashMap::new();

        for (field_position, field_name) in field_names.iter().enumerate() {
            let field_vec = match columns[field_position].as_array() {
                Some(v) => v.iter().filter_map(|v| v.as_u64()).collect(),
                _ => return Err(anyhow!("Edge data must be a json array with numeric values")),
            };
