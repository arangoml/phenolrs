...
pub async fn get_all_shard_data(
    ...
    for (server, shard_list) in shard_map.iter() {
        ...
        let body = DumpStartBody {
            batch_size: req.configuration.load_config.batch_size.unwrap(),
            prefetch_count: 5,
            parallelism: req.configuration.load_config.parallelism.unwrap(),
    ...
    let par_per_dbserver = if dbservers.is_empty() {
        ...
        (req.configuration.load_config.parallelism.unwrap() as usize + dbservers.len() - 1)
            / dbservers.len()
    };
